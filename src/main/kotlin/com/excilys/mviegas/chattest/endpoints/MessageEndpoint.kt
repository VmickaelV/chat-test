...
class MessageEndpoint(private val messageDao: MessageDao) {
    ...
    @GetMapping
    fun getMessages(@ApiIgnore @AuthenticationPrincipal account: Account, pageable: Pageable, @RequestParam("date") optionalLocalDateTime:Optional<LocalDateTime>): Page<MessageDto> {
        return messageDao.findAllByDateLessThanEqualOrderByDateDesc(optionalLocalDateTime.orElse(LocalDateTime.now()), pageable).map { MessageDto(it) }
    }
